...
const HeroSection = () => {
    ...
    (async () => {
      try {
        const [gr, sj] = await Promise.all([
          fetch("/api/meta/grades", { cache: "no-store" }).then((r) => r.json()),
          fetch("/api/meta/subjects", { cache: "no-store" }).then((r) => r.json()),
        ]);
  ...
  // ---- Helpers: sanitize text and compute relevance score ----
  const toPlain = (s) =>
  ...
  const scoreByQuery = (obj, query) => {
    ...
    const content = toPlain(
      obj.presentation_content || obj.presentation_html || obj.content || ""
    );
    const hit = (text) => ({ starts: text.startsWith(ql), includes: text.includes(ql) });
    let score = 0;
    const t = hit(topic),
      st = hit(subtopic),
      sj = hit(subject),
  ...
  // Debounced suggestions fetch — get top 4 suggestions by relevance
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const sorted = rawItems.sort(
          (a, b) => scoreByQuery(b, query) - scoreByQuery(a, query)
        );
  ...
  // 🚩 Use URLSearchParams to build clean query strings that Explore Library can read.
      ...
      <nav className="flex flex-wrap items-center justify-between md:ml-8 md:mr-7 md:mt-2 md:py-4 sm:px-30 md:px-0 lg:px-8 xl:mb-10 text-white">
        ...
        <ul className="font-inter hidden lg:flex flex-grow justify-center md:text-[12px] md:gap-4 text-sm lg:text-[14px] lg:gap-4">
          <li className="hover:bg-[#9500DE] px-2 py-2">
            <Link
              ...
              className="transition hover:text-gray-200 cursor-pointer"
            >
              Explore Library
            </Link>
          </li>
              ...
              <ul className="absolute left-0 top-full mt-1 w-48 bg-gray-800 rounded-md shadow-lg z-[500]">
                {subjects.map((subject, idx) => (
                  <li
                    key={idx}
                    className="px-4 py-2 text-white border-b border-gray-700 last:border-none hover:bg-[#9500DE] cursor-pointer"
                    onClick={() => onClickSubject(subjects)}
                  >
                    {subject}
                  </li>
                ))}
        ...
        <div className="flex items-center gap-7 lg:hidden">
          <div className="hidden md:flex items-center gap-6">
            <Link
              href="/login"
              className="transition hover:text-gray-200 text-sm md:text-[12px]"
            >
              Login
            </Link>
            <span className="hidden">
      ...
      <div className="relative flex flex-col items-center justify-center px-0 text-center md:flex-row md:text-left min-h-[370px]">
        ...
        <div className="relative z-[300] mt-8 flex w-full flex-col items-center space-y-3 md:mt-10 md:mb-10 md:justify-center xl:space-y-6">
          ...
          >
            ...
            {open && (
              <div className="absolute left-0 top-[110%] mt-2 w-full bg-gray-900 text-white rounded-xl shadow-2xl z-[400]">
                {loading && (
                ...
                {!loading && items.length > 0 && (
                  <div className="divide-y divide-gray-800">
                    {items.map((item) => {
                      const topic = item.topic || item.title || item.name || "Untitled";
                      const grade = item.grade || "";
                      const subtopic = item.sub_topic || item.subtopic || "";
                      const snippet = makeSnippet(
                        item.snippet ||
                          item.presentation_content ||
                          item.presentation_html ||
                          item.content
                      );
                      return (
                        <Link
                        ...
                        >
                          ...
                          {subtopic && (
                            <div className="text-xs opacity-80">{subtopic}</div>
                          )}
                          {snippet && (
